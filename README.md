...
Other env vars: `TABLM_AUTH_TOKEN`, `TABLM_GATEWAY_URL`, `TABLM_GATEWAY_HTTP`, `TABLM_GATEWAY_HOST`, `TABLM_GATEWAY_TOKEN`, `TABLM_DEFAULT_SITE`, `TABLM_MAX_REPAIRS`, `TABLM_MAX_CALLS_PER_TURN` (6), `TABLM_MAX_RESULT_CHARS` (8000), `TABLM_CHAT_ROLLOVER_CHARS` (400000 - chars pasted into one web chat before it rolls over to a fresh one), `TABLM_TRANSPORT`, `TABLM_NO_LOCAL_CHROME`, `TABLM_CHROME_PROFILE`, `TABLM_BRIDGE_PORT`, `TABLM_BRIDGE_TOKEN`, `TABLM_SITES_CONFIG`, `TABLM_SESSIONS`
